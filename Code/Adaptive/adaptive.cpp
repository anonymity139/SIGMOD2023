...
void Adaptive(point_set *origset, point_t* u, int &Qcount, int mode)
{
    ...
    else
    {
        ...
        {
            int ide = rand() % origset->points.size();
            bool is_same = false;
            for (int j = 0; j < p_set->points.size(); j++)
            {
                if (p_set->points[j]->is_same(origset->points[ide]))
                {
                    is_same = true;
                    break;
                }
            ...
            if (!is_same)
            {
                p_set->points.push_back(origset->points[ide]);
                cco++;
                if (cco >= 1000)
                    break;
            }
    ...
    while (accuracy < 0.99999 && Qcount < 100 && de_accuracy > 0 )
    {
        Qcount++;
        ...
        double ac = u->cosine0(estimate_u);
    ...
    origset->points[maxIdx]->printResult(out_cp, "Adpative", Qcount, t1, mode);
    out_cp.close();
}
...
void Adaptive_prune(point_set *origset, point_t* u, int &Qcount, int mode)
{
    ...
    int M;
    double totalSize = origset->points.size();
    point_set *C = new point_set(origset);
    //p_set: randomly choose 1000 points
    point_set *p_set = new point_set();
    if (origset->points.size() < 1000)
    {
    ...
    point_t *estimate_u = find_estimate(V);
    estimate_u->normalization();
    while (C->points.size() > 1 && accuracy < 0.99999 && Qcount < 100 && de_accuracy > 0 )
    {
        Qcount++;
        hyperplane *best = NULL;
        best = orthogonal_search(stree_root, estimate_u, best);
        ...
        if (v1 >= v2)
        {
            for (int i = 0; i < dim; ++i)
                pt->attr[i] = best->norm[i];
            R->hyperplanes.push_back(new hyperplane(q, p, 0));
            C->prune(q);
        }
        else
        {
            for (int i = 0; i < dim; i++)
                pt->attr[i] = -best->norm[i];
            R->hyperplanes.push_back(new hyperplane(p, q, 0));
            C->prune(p);
        }
        V.push_back(pt);
        estimate_u = find_estimate(V);
    ...
    origset->points[maxIdx]->printResult(out_cp, "Adpative_Prune", Qcount, t1, mode);
    out_cp.close();
}