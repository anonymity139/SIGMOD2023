...
// frame: the frame for obtaining the set of neibouring vertices of the current best vertiex (used only if cmp_option = SIMPLEX)
// cmp_option: the car selection mode, which must be either SIMPLEX or RANDOM
void update_ext_vec(tuple_set *car_set, point_set* P, vector<int>& C_idx, point_t* u, int s, vector<point_t*>& ext_vec,
        int& current_best_idx, int& last_best, vector<int>& frame, int cmp_option, timeval &t1, timeval &t2, double &totalTime)
{
	// generate s cars for selection in a round
	...
	for(int i = 0; i < S.size() - 1; i++)
	{
        ...
        */
        ...
        int option = car_set->show_to_user(P->points[ C_idx[S[0]] ]->id, P->points[ C_idx[S[1]] ]->id);
...
point_t* max_utility(tuple_set *car_set, point_set* P, point_t* u, int s,  double epsilon, int maxRound, int &Qcount, double &Csize,  int cmp_option, int stop_option, int prune_option, int dom_option, int &TID, std::ofstream &fp)
{
    timeval t1, t2;
    gettimeofday(&t1, 0);
	int dim = P->points[0]->d;
	...
	while (C_idx.size()> 1 && (rr > epsilon  && !isZero(rr - epsilon)) && Qcount <  maxRound)  // while none of the stopping conditiong is true
	{
		...
		// generate the options for user selection and update the extreme vecotrs based on the user feedback
		update_ext_vec(car_set, P, C_idx, u, s, ext_vec, current_best_idx, last_best, frame, cmp_option, t1, t2, totalTime);
    ...
    if(cmp_option == SIMPLEX)
        car_set->tuples[result->id]->final_result("UH-Simplex", Qcount, totalTime, fp);
    else
        car_set->tuples[result->id]->final_result("UH-Random", Qcount, totalTime, fp);
    TID = result->id;
