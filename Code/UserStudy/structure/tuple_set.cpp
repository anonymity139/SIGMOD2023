...
point_set* tuple_set::transform(std::vector<std::string> &categorical)
{
    ...
    for(int i = 0; i < size; i++)
    {
        p = new point_t(index + dim_num, tuples[i]->id);
...
/**
 * Show the user two tuples
 * @param P
 * @param p_idx
 * @param q_idx
 * @return
 */
int tuple_set::show_to_user(int idx_1, int idx_2)
{
    ...
    while (option != 1 && option != 2)
    {
        ...
        std::cout << "-------------------------------------------------------------------------\n";
        std::cout << std::setw(10) << "Option 1" << std::setw(10) << tuples[idx_1]->attr_cat[0] << std::setw(12) << tuples[idx_1]->attr_num[0] << std::setw(12) << tuples[idx_1]->attr_num[1] << std::setw(12) << tuples[idx_1]->attr_num[2] << std::setw(12) << tuples[idx_1]->attr_num[3] <<"\n";
        std::cout << "-------------------------------------------------------------------------\n";
        std::cout << std::setw(10) << "Option 2" << std::setw(10) << tuples[idx_2]->attr_cat[0] << std::setw(12) << tuples[idx_2]->attr_num[0] << std::setw(12) << tuples[idx_2]->attr_num[1] << std::setw(12) << tuples[idx_2]->attr_num[2] << std::setw(12) << tuples[idx_2]->attr_num[3] <<"\n";
        std::cout << "-------------------------------------------------------------------------\n";
        printf("Your choice: ");
        std::cin >> option;
    }
...
void tuple_set::random_choose(tuple_set* tset)
{
    int count = 0;
    while (count < 1000)
    {
        int index = rand() % tuples.size();
        if(   1000 < tuples[index]->attr_num[0] && tuples[index]->attr_num[0] < 100000
           && 2001 < tuples[index]->attr_num[1] && tuples[index]->attr_num[1] < 2017
             && 50 < tuples[index]->attr_num[2] && tuples[index]->attr_num[2] < 1000
           && 5000 < tuples[index]->attr_num[3] && tuples[index]->attr_num[3] < 150000)
        {
            bool is_same = false;
            for (int j = 0; j < tset->tuples.size(); j++)
            {
                if (tuples[index]->is_same(tset->tuples[j]))
                {
                    is_same = true;
                    break;
                }
            ...
            if (!is_same)
            {
                tuple_t * tt = new tuple_t(tuples[index]);
                tset->tuples.push_back(tt);
                ++count;
            }
        }
    }
...
void tuple_set::preprocessCar()
{
    ...
    for (int i = 0; i < tuples.size(); ++i)
    {
        for (int j = 0; j < d_num; j++)
        {
            if (max[j] < tuples[i]->attr_num[j])
                max[j] = tuples[i]->attr_num[j];
            if (min[j] > tuples[i]->attr_num[j])
                min[j] = tuples[i]->attr_num[j];
        }
    }
    for (int i = 0; i < tuples.size(); i++)
    {
        for (int j = 0; j < d_num; j++)
        {
            if (j == 0 || j == 3)
            {
                tuples[i]->attr_num[j] = 1 - (tuples[i]->attr_num[j] / max[j]);
            }
            else
            {
                tuples[i]->attr_num[j] = tuples[i]->attr_num[j] / max[j];
            }
        }
    }
}
